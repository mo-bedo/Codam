...
AForm::AForm () : _name("default"), _signed(false), _req_sign_grade(AForm::_highest_grade),
					 _req_exec_grade(AForm::_highest_grade)
{
	reviewAForm();
...
AForm::AForm (const AForm &obj) : _name(obj.getName()), _signed(obj.getSignedStatus()), 
			_req_sign_grade(obj.getReqSignGrade()),	_req_exec_grade(obj.getReqExecGrade())
{
	reviewAForm();
	if (AFORM_PRINT_CONSTRUCT_MSG)
...
AForm& AForm::operator= (const AForm &obj)
{
	...
	reviewAForm();
	if (AFORM_PRINT_CONSTRUCT_MSG)
		std::cout << "Copy assignment operator AForm called. Constant values are assigned default values. Don't use this." << std::endl;
	return *this;
}
